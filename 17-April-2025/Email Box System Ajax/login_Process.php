		...
		extract($_REQUEST);
			...
			if($result->num_rows){
				$row = mysqli_fetch_array($result);
					...
					setcookie("user_id",base64_encode($row['user_id']),time()+(60*30),"/","localhost",true,true);
					setcookie("username",base64_encode($row['name']),time()+(60*30),"/","localhost",true,true);
					setcookie("email",base64_encode($row['email']),time()+(60*30),"/","localhost",true,true);
					setcookie("password",base64_encode($row['password']),time()+(60*30),"/","localhost",true,true);
