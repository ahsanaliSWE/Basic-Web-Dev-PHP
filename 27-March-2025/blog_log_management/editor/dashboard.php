<?php
    ...
    if(isset($_GET['edit'])){
        $post_id = $_GET['edit'];
        $query2 = "SELECT title, summary, description FROM posts WHERE post_id = ? AND added_by = ?";
        $statement = mysqli_prepare($database_connection, $query2);
        mysqli_stmt_bind_param($statement, "ii", $post_id, $_SESSION['user_id']);
        ...
        if(isset($_POST['update_post'])){
            ...
            $update_query = "UPDATE posts SET title = ?, summary = ?, description = ? WHERE post_id = ? AND added_by = ?";
            $statement = mysqli_prepare($database_connection, $update_query);
            mysqli_stmt_bind_param($statement, "sssii", $post_title, $post_summary, $post_description, $post_id, $_SESSION['user_id']);
    ...
    if(isset($_GET['delete'])){
        $post_id = $_GET['delete'];
        $query = "DELETE FROM posts WHERE post_id = ? AND added_by = ?";
        $statement = mysqli_prepare($database_connection, $query);
        mysqli_stmt_bind_param($statement, "ii", $post_id, $_SESSION['user_id']);
        $result = mysqli_stmt_execute($statement);
    ...
    $per_page = 3;
    $query_1 = "SELECT COUNT(post_id) AS total_records FROM posts WHERE added_by = ?";
    $statement = mysqli_prepare($database_connection, $query_1);
    mysqli_stmt_bind_param($statement, "i", $_SESSION['user_id']);
    mysqli_stmt_execute($statement);
    mysqli_stmt_bind_result($statement, $total_records);
    mysqli_stmt_fetch($statement);
    mysqli_stmt_close($statement);
    ...
    $query = "SELECT p.post_id, p.title, p.summary, p.description, u.name FROM posts p JOIN user u ON p.added_by = u.user_id WHERE p.added_by = ? LIMIT ?, ?";
    $statement = mysqli_prepare($database_connection, $query);
    mysqli_stmt_bind_param($statement, "iii", $_SESSION['user_id'], $start, $per_page);
    mysqli_stmt_execute($statement);
...
<body>
    <header>
        <h1>Welcome <?= $_SESSION['name']; ?> to Editor Dashboard</h1>
           <div class="logout">
