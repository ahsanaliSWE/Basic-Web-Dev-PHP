    ...
    if (!$database_connection) {
        ...
        if (isset($_POST['register'])) {
            ...
            $role_type_id = $_POST['role_type_id'];
                ...
                $query = "INSERT INTO user (name, email, password, gender, role_type_id) VALUES (?, ?, ?, ?, ?)";

                $statement = mysqli_prepare($database_connection, $query);
        
                mysqli_stmt_bind_param($statement, "ssssi", $name, $email, $password, $gender, $role_type_id);

        	    $result = mysqli_stmt_execute($statement);

        
                if(isset($name) && isset($email) && isset($password) && isset($gender) && isset($role_type_id)) {
                    if($result){
            ...
            if(isset($_POST['login'])){
                ...
                if($role_type_id !=1){
                    ...
                    $log_query = "INSERT INTO log (login, user_id) VALUES (?, ?)";
                    
                    $log_statement = mysqli_prepare($database_connection, $log_query);
                    
                    $login = date('Y-m-d H:i:s');
                    mysqli_stmt_bind_param($log_statement, "si", $login, $user_id);
